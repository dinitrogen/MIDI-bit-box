...
static const ushort MAX_TONES = 9;
static const ToneParams tones[MAX_TONES] = {
  { 5, 5, 5 },
  { 5, 5, 10 },
  { 5, 5, 15 },
...
void playMelody(int *melodyNotes, int *melodyLengths) {
  delay(1000);
  midiEventPacket_t rx = MidiUSB.read();

  int len = sizeof(melodyNotes);
  
  for (int i = 0; i < len; i++) {  
    noteOn( 144 & 0xF, melodyNotes[i], 60 );
    psg.update();
    delay(melodyLengths[i]);
    noteOn( 144 & 0xF, melodyNotes[i], 0 );
    psg.update();
  }
}
...
int testMelodyNotes[] = {71, 69, 67, 71};
int testMelodyLengths[] = {200, 400, 400, 400};

//playMelody(testMelodyNotes, testMelodyLengths);
...
int whole = 1600;
  ...
  noteOn( 144 & 0xF, 71, 60 );
  psg.update();
  delay(eigth);
  noteOn( 144 & 0xF, 71, 0 );
  psg.update();
  
  noteOn( 144 & 0xF, 69, 60 );
  psg.update();
  delay(quarter);
  noteOn( 144 & 0xF, 69, 0 );
  psg.update();
  
  noteOn( 144 & 0xF, 67, 60 );
  psg.update();
  delay(quarter);
  noteOn( 144 & 0xF, 67, 0 );
  psg.update();
  ...
  noteOn( 144 & 0xF, 71, 80 );
  psg.update();
  delay(quarter);
  noteOn( 144 & 0xF, 71, 0 );
  psg.update();

  noteOn( 144 & 0xF, 72, 60 );
  psg.update();
  delay(quarter);
  noteOn( 144 & 0xF, 72, 0 );
  psg.update();    
